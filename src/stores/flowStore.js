...
import { FlowEngine, TypeEngine, AutoExecutionEngine, deepClone } from '@flow-forge/core'
import { executeNode as frontendExecuteNode, extractOutputs } from '../engine/executor'
import { loadDataTypes, detectValueType } from '@flow-forge/core/engine/dataTypes.js'
...
export const useFlowStore = defineStore('flow', () => {
  ...
  const saveExecutionToHistory = async (executionData) => {
    const token = apiConfig.value.token
    const baseUrl = apiConfig.value.baseUrl || 'http://localhost:3001'

    if (!token) {
      throw new Error('No authentication token')
    }

    const response = await fetch(`${baseUrl}/api/executions/save`, {
      method: 'POST',
      headers: {
    ...
    if (!response.ok) {
      ...
      throw new Error(error.error?.message || 'Failed to save execution')
    }
  ...
  const executeFlowOnBackend = async (flowId = null, inputData = {}, options = {}) => {
    ...
    if (!token) {
      throw new Error('Token de autenticação não configurado. Configure apiConfig.token')
    }
    ...
    try {
      ...
      const response = await fetch(`${baseUrl}/api/flows/${targetFlowId}/execute`, {
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json'
      ...
      if (result.success) {
        ...
        throw new Error(result.message || 'Erro desconhecido na execução')
      }
  ...
  const syncToAPI = async () => {
    ...
    try {
      ...
      const response = await fetch(
        `${apiConfig.value.baseUrl}/api/flow-data/${apiConfig.value.flowDataId}`,
        {
          method: 'PUT',
          headers: {
            'Content-Type': 'application/json',
      ...
      // 🚀 NOVO: Marcar como salvo e atualizar estado de referência
  ...
  const loadFromAPI = async (flowDataId) => {
    try {
      const response = await fetch(
        `${apiConfig.value.baseUrl}/api/flow-data/${flowDataId}`,
        {
          headers: {
            'Authorization': `Bearer ${apiConfig.value.token}`
          }
  ...
  const fetchFlowData = async () => {
    ...
    try {
      const response = await fetch(
        `${apiConfig.value.baseUrl}/api/flow-data/by-flow/${apiConfig.value.flowId}`,
        {
          method: 'GET',
          headers: {
            'Authorization': `Bearer ${apiConfig.value.token}`,
