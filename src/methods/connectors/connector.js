...
export async function execute({ nodeData, inputs, context }) {
  ...
  const coreConnector = await import('@flow-forge/core/methods/connectors/connector.js')
  const { AdapterRegistry } = await import('@flow-forge/core/engine/AdapterRegistry.js')
...
export async function validate(nodeData) {
  ...
  // Nota: @flow-forge/core já aponta para ../core/src (ver vite.config.js)
  const coreConnector = await import('@flow-forge/core/methods/connectors/connector.js')
